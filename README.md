...
2. **External Data Sources**: The knowledge maps in this repository were built from two external databases:
   ...
   - **Environmental footprint dataset**: carbon (gCO₂e/100 g), water (L/100 g), and ecological (points/100 g) footprints. Available at [link to be added].
...
To reproduce the optimization results:

1. Load base diets from `diets-base/`
2. Use maps from `maps/` for food identification and nutritional/environmental data lookup
3. Run GA or LP optimizers with the hyperparameters documented in `formulations/`
4. Compare results against the corresponding `optimized-diets/` subdirectory

All source code for the optimization pipeline is available in the main repository.
...
```
[Authors], "Neuro-Symbolic Optimization for a Sustainable Diet Recommendation
System under Climate Emergency", SEMISH 2026.
```
