...
class ArtifactPopulationTests(unittest.TestCase):
    ...
    def test_pending_mapping_exclusion_config_is_exact_and_scenario_only(self) -> None:
        ...
        self.assertEqual(len(set.union(*exclusions.values())), 12)
        self.assertEqual(tuple(len(exclusions[profile]) for profile in ("regular", "vegetariana", "vegana")), (2, 8, 7))
    ...
    def test_mapping_exclusion_sensitivity_removes_exact_source_occurrences(self) -> None:
        ...
        expected_incremental = {"regular": 23, "vegetariana": 130, "vegana": 145}
...
class CommandCatalogTests(unittest.TestCase):
    ...
    def test_profile_ingredient_audit_marks_only_configured_exact_exclusions(self) -> None:
        ...
        self.assertEqual((len(rows), len(evidenced), len(pending)), (364, 6, 356))
...
class BaseDietAuditTests(unittest.TestCase):
    def test_all_150_base_diets_are_auditable_and_mapped(self) -> None:
        ...
        self.assertEqual(len(adjudication["source_food_names"]), 158)
        self.assertEqual(totals["non_identity_mappings_with_recorded_target_decision"], 158)
        self.assertEqual(totals["non_identity_mappings_without_recorded_target_decision"], 12)
        self.assertEqual(len(mapping_lines), totals["unique_food_names"] + 1)
        self.assertEqual(len(current_queue), 12)
