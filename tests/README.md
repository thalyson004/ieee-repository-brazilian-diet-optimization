...
Latest audit refresh (2026-09-24): 133 target decisions are indexed and the active queue has 37 unresolved links. The latest audit is `base-diet-audit_20260924T190506128701Z_a1efb817`; the queue refresh is `mapping-review-queue-current_20260924T190506434213Z_df6bd1d5`. The 28-test suite passed. The integrated rerun (GA smoke, LP scope/slack, missingness, ingredient-screen, portion-support, and 10-seed replication) passed; see below. Ground-beef oil type, walnut/pecan identity, recipe formulation, ingredients, and environmental pairings remain unresolved.

Latest ten-seed rerun after the unsweetened mint-infusion correction: `full-replication_20260924T190507905039Z_d8734870`, 66 outputs (60 GA + 6 LP), seed-base 20260928, 374.1 s, `status=passed`; scope validation passed with no items outside profile pools. The complete nine-command battery passed. This run predates commit/input hashes in the manifest and remains diagnostic; a provenance-enabled rerun is next. No result is approved for the manuscript; see `EXPERIMENT-EXECUTION-PLAN.md` for the blockers.
