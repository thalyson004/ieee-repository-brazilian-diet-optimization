...
The current code audit is regenerated from the active maps, diets, and dated adjudication index. Latest checkpoint `base-diet-audit_20260924T181426319753Z_7d1a005b` reports 122 identity mappings, 117 non-identity targets with a recorded decision, and 53 without one (170 non-identity links total). One active target is shared by multiple source names; the audit found zero unmapped TBCA/environmental occurrences and 1,581 target violations under the diagnostic calculation. The violation count is sensitive to map changes and is not a quality score. The preceding checkpoint `base-diet-audit_20260924T164329756813Z_5bb00410` is a historical 41/129 snapshot.
