...
The current code audit is regenerated from the active maps, diets, and dated adjudication index. Run `python -m tests.run_experiments --experiments base-diet-audit`; checkpoint `base-diet-audit_20260924T163815669498Z_8a786c88` reports 122 identity mappings, 40 non-identity targets with a recorded decision, and 130 without one (170 non-identity links total). Four active targets are shared by multiple source names; the audit found zero unmapped TBCA/environmental occurrences and 1,563 target violations under the diagnostic calculation. The violation count is sensitive to map changes and is not a quality score.
...
- The original map preserved no confidence score, reviewer identity, or target-selection rationale. The revised dated ledger now records 40 target decisions and their sources; it is not a complete record of the remaining 130 links or of all environmental equivalence questions.
