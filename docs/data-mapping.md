...
Update, 2026-09-24: the frozen initial queue has 170 non-identity links. Thirty-nine nutrition-target decisions are now documented; 131 other original links remain unadjudicated. This does not establish ingredient or preparation equivalence, environmental-coefficient validity, or complete formulation matching. In particular, oil type, tofu salt, fluid-milk processing, coconut processing/maturity, pasta refinement, rice-cracker formulation, vegetable-risotto ingredients, toast brand/formulation, and couve variety remain uncertain; see the dated correction ledger below. The salted-couscous target now excludes butter and matches the stated steamed preparation, although serving/formulation details should still be checked against the original source. Where later text in this document reports older review counts, treat those as historical snapshots superseded by this update and the dated ledger.

This document describes the exact mapping path used by the archived submission and the checks required by the revised study. It distinguishes exact identity, a non-identity link with a dated target decision, and a non-identity link still awaiting a target decision. The old artifacts do not classify all links as lexical normalization versus semantic substitution. Thirty-nine target decisions are indexed in [`archive/audits/adjudicated-food-map-sources.json`](../archive/audits/adjudicated-food-map-sources.json) and documented item by item in [`archive/audits/food-mapping-corrections-2026-09-24.md`](../archive/audits/food-mapping-corrections-2026-09-24.md); residual preparation, brand/variety, environmental, or ingredient uncertainty can remain even for those decisions.
...
The current code audit is regenerated from the active maps, diets, and dated adjudication index. Run `python -m tests.run_experiments --experiments base-diet-audit`; checkpoint `base-diet-audit_20260924T163228253412Z_31785200` reports 122 identity mappings, 39 non-identity targets with a recorded decision, and 131 without one (170 non-identity links total). Four active targets are shared by multiple source names; the audit found zero unmapped TBCA/environmental occurrences and 1,562 target violations under the diagnostic calculation. The violation count is sensitive to map changes and is not a quality score.
...
- The original map preserved no confidence score, reviewer identity, or target-selection rationale. The revised dated ledger now records 39 target decisions and their sources; it is not a complete record of the remaining 131 links or of all environmental equivalence questions.
