...
Latest correction: the homemade mozzarella-pizza source is now linked to TBCA BRC0225A (artisanal baked mozzarella pizza), not the previous Marguerita record BRC0685A. The closer target removes the unsupported basil and milk/egg/margarine dough assumptions, though it remains provisional because the source recipe is unavailable. A generic cereal-matinal target was also verified, retaining unresolved brand/sugar detail. The index has 67 decisions and the current queue has 103 pending links.
...
The current code audit is regenerated from the active maps, diets, and dated adjudication index. Run `python -m tests.run_experiments --experiments base-diet-audit`; checkpoint `base-diet-audit_20260924T164329756813Z_5bb00410` reports 122 identity mappings, 41 non-identity targets with a recorded decision, and 129 without one (170 non-identity links total). Four active targets are shared by multiple source names; the audit found zero unmapped TBCA/environmental occurrences and 1,561 target violations under the diagnostic calculation. The violation count is sensitive to map changes and is not a quality score.
...
- The original map preserved no confidence score, reviewer identity, or target-selection rationale. The revised dated ledger now records 41 target decisions and their sources; it is not a complete record of the remaining 129 links or of all environmental equivalence questions.
